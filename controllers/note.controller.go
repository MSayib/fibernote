...
import (
	...
	"github.com/msayib/fibernote/models"
	"gorm.io/gorm"
)
...
func Store(c *fiber.Ctx) error {
	...
	if result.Error != nil && strings.Contains(result.Error.Error(), "duplicate key value violates unique") {
		return c.Status(fiber.StatusConflict).JSON(fiber.Map{"status": "error", "message": "Title already exist, please use another title"})
...
func Update(c *fiber.Ctx) error {
	...
	if err := result.Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return c.Status(fiber.StatusNotFound).JSON(fiber.Map{"status": "error", "message": "No note with that Id exists"})
		}
...
func Destroy(c *fiber.Ctx) error {
	...
	return c.Status(fiber.StatusOK).JSON(fiber.Map{"status": "success", "message": "Note deleted successfully"})
}
